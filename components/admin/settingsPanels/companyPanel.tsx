...
export default function CompanyPanel() {
  ...
  // Hämta deals (kopplade till companyId == user.uid)
  useEffect(() => {
    if (!user) return;
    (async () => {
      setFetching(true);
      ...
      setExpiredDeals((mapped as Deal[]).filter((deal) => deal.expiresAt <= now));
      setFetching(false);
    })();
  }, [user]);
  ...
  const refreshStripeStatus = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/create-stripe-account?${params.toString()}`, {
        method: 'GET',
      });

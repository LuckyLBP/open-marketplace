  ...
  return (
    <Card className="overflow-hidden transition-all hover:shadow-lg h-full flex flex-col group relative">
      <Link href={`/product/${id}`} className="group relative">
        ...
        {isOnSale && (
          <Badge className="absolute top-2 left-2 bg-red-600 hover:bg-red-700">-{discountPercentage}%</Badge>
        )}
      </Link>
      <CardHeader className="pb-2 pt-3">
        <div className="flex flex-col">
          <div className="flex justify-start gap-1.5 mb-2">
            <Badge variant="outline" className="bg-purple-50 whitespace-nowrap text-xs">
              {duration}h
            </Badge>
          ...
          <div>
            <Link href={`/product/${id}`}>
              <CardTitle className="text-base hover:text-purple-600 transition-colors line-clamp-2 min-h-[2.5rem]">{title}</CardTitle>
            </Link>
      ...
      <CardContent className="pb-3 pt-0 flex-grow flex flex-col">
        <p className="text-xs text-muted-foreground mb-auto min-h-[3rem]">{description}</p>

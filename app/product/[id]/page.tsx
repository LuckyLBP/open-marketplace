...
import { ArrowLeft, Clock, ShoppingCart, Store, Heart, Eye, Share2, Check, Star } from "lucide-react"
...
export default function ProductDetail({ params }: { params: { id: string } }) {
  ...
  const handleAddToCart = () => {
    toast({
      ...
      description: `${product?.title} (${quantity}) ${t("has been added to your cart")}`,
    })
  }
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-grow flex flex-col items-center justify-center p-4">
          <div className="text-center max-w-md">
            ...
            <p className="mb-6 text-muted-foreground">{t("This product may have expired or been removed.")}</p>
            ...
            <div>
              <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                  ...
                  {product.inStock ? (
                    <div className="flex items-center text-green-600">
                      ...
                      <span className="font-medium">
                        {t("In Stock")} ({product.stockQuantity} {t("available")})
                      </span>
                    </div>
                  ) : (
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setQuantity(
                          Math.min(product.stockQuantity || 99, Math.max(1, Number.parseInt(e.target.value) || 1)),
                        )
                      }
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="icon" className="h-12 w-12" onClick={handleAddToWishlist}>
                        ...
                        <div className="grid gap-4 py-4 md:grid-cols-2">
                          ...
                          <div>
                            ...
                            <div className="mb-2">
                              ...
                              <span className={cn("text-lg font-bold", isOnSale && "text-red-600")}>
                                {new Intl.NumberFormat("sv-SE", { style: "currency", currency: "SEK" }).format(
                                  product.price,
                                )}
                              </span>
              ...
              <div className="bg-white rounded-lg shadow-md p-4 mb-6">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-muted-foreground">{t("SKU")}:</span> {product.sku}
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-muted-foreground">{t("Transaction Fee")}:</span> {product.feePercentage}%
                  </div>
                  <div>
                    <span className="text-muted-foreground">{t("Duration")}:</span> {product.duration} {t("hours")}
                  </div>
                </div>
              </div>
              ...
              {product.features && product.features.length > 0 && (
                <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                  <h2 className="text-xl font-bold mb-4">{t("Key Features")}</h2>
                  ...
                  <div className="flex items-center mb-6">
                    <div className="flex mr-4">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          className={cn("h-5 w-5", star <= 4 ? "text-yellow-400 fill-yellow-400" : "text-gray-300")}
                        />
                      ))}
                    </div>
                      ...
                      <div className="flex items-center mb-2">
                        <div className="flex mr-2">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                              key={star}
                              className={cn("h-4 w-4", star <= 5 ? "text-yellow-400 fill-yellow-400" : "text-gray-300")}
                            />
                          ))}
                        </div>
                        <span className="font-medium">Anna L.</span>
                        <span className="text-muted-foreground text-sm ml-2">2023-10-15</span>
                      </div>
                      <h3 className="font-medium mb-1">Great product, highly recommend!</h3>
                      <p className="text-muted-foreground">
                        I've been using this for a month now and I'm very impressed with the quality. The sound is
                        crystal clear and the battery life is amazing.
                      </p>
                    </div>
                    <div className="border-b pb-6">
                      <div className="flex items-center mb-2">
                        ...
                        <span className="text-muted-foreground text-sm ml-2">2023-09-22</span>
                      </div>
                      <h3 className="font-medium mb-1">Good but could be better</h3>
                      <p className="text-muted-foreground">
                        The product works well overall, but I had some issues with the Bluetooth connection. Sometimes
                        it disconnects randomly. Otherwise, it's a good product for the price.
                      </p>
                    </div>
                    <div>
                      <div className="flex items-center mb-2">
                        <div className="flex mr-2">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                              key={star}
                              className={cn("h-4 w-4", star <= 3 ? "text-yellow-400 fill-yellow-400" : "text-gray-300")}
                            />
                          ))}
                        </div>
                        <span className="font-medium">Maria S.</span>
                        <span className="text-muted-foreground text-sm ml-2">2023-08-05</span>
                      </div>
                      <h3 className="font-medium mb-1">Decent but overpriced</h3>
                      <p className="text-muted-foreground">
                        The product is decent, but I think it's a bit overpriced for what it offers. The build quality
                        is good, but I expected more features for this price point.
                      </p>
                    </div>
                  </div>
                </TabsContent>

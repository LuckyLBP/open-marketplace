...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!dealId || !type || !duration || !price) {
            return NextResponse.json({ error: "Ogiltig begäran  saknar fält" }, { status: 400 });
        }
        ...
        if (!dealSnap.exists()) {
            return NextResponse.json({ error: "Erbjudandet finns inte" }, { status: 404 });
        }
        ...
        return NextResponse.json({ error: "Checkout misslyckades" }, { status: 500 });
    }
}

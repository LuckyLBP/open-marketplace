...
export async function POST(req: Request) {
  try {
    ...
    const accountType: AccountType =
      body?.accountType === 'customer' ? 'customer' : 'company';

    if (!sellerId) {
      return NextResponse.json(
        { error: 'sellerId (eller entityId) krävs' },
        { status: 400 }
      );
    }
    ...
    const accountLink = await stripe.accountLinks.create({
      ...
      refresh_url: `${baseUrl}/dashboard/settings?connect=refresh&aid=${encodeURIComponent(
        sellerId
      )}`,
      return_url: `${baseUrl}/dashboard/settings?connect=done&aid=${encodeURIComponent(
        sellerId
      )}`,
...
export async function GET(req: Request) {
  try {
    ...
    const accountType: AccountType =
      (searchParams.get('accountType') as AccountType) === 'customer'
        ? 'customer'
        : 'company';
    ...
    return NextResponse.json(
      { error: err?.message || 'Ett fel uppstod vid statuskontroll' },
      { status: 500 }
    );
  }
}

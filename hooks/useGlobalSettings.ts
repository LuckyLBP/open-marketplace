...
export type ServiceFees = Record<string, number>; // t.ex. { "12":3, "24":4, ... }
export type BoostPrices =
  | { floatingPerHour?: number; bannerPerHour?: number } // per-timme
  | { floating?: Record<string, number>; banner?: Record<string, number> }; // per-duration
...
export function useGlobalSettings() {
  ...
  useEffect(() => {
    ...
    // live-lyssning (smidigt när superadmin ändrar)
...
// Hjälpare: hämta fee% från map, fallback till närmsta definierade/”tier”
// Ex: om 168 h saknas men 144=10 finns, använd 10.
export function feeForDuration(serviceFees: ServiceFees, duration: number): number {
  ...
  // fallback: ta närmsta lägre nyckel, annars högsta
  const keys = Object.keys(serviceFees).map((k) => parseInt(k, 10)).sort((a, b) => a - b);
...
// Hjälpare: boost-pris från settings (stöder per-hour eller per-duration-tabeller)
export function boostPriceFor(boost: BoostPrices, placement: 'floating'|'banner', duration: number): number {
  // per-duration-tabell (om finns)
